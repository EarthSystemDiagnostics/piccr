...
##' Prepare standard calibration
##'
##' Wrapper function for the drift-correction and the linear calibration of the
##' standard isotope data.
##' @param std_data a data frame with only the data of the standard samples for
##' a specific measurement file.
##' @param meta.info_std the respective sublist of the output from
##' \code{\link{get.input_folder}}.
##' @inheritParams CalibratePicarroData
##' @return a list structure with the calibrated standard data along with
##' statistical information on the quality of the calibration and drift
##' correction approach.
##' @author Thomas Münch
wrapper.StandardCalibration <- function(std_data, meta.info_std,
                                        flag.Calibration) {
    ...
    if (flag.Calibration!=0 && flag.Calibration!=1 && flag.Calibration!=2)
        stop('Unknown value of flag.Calibration in function
    ...
    res$sd.residuals$delta.O18=
        res.calibration_delta.O18$sd.residuals_FirstBlockCalib
    res$sd.residuals$delta.H2=
        res.calibration_delta.H2$sd.residuals_FirstBlockCalib
...
##' Calibration of standard data
##'
##' Function to perform a linear calibration of the measured standard isotope
##' data for a specific isotope species given literature delta values for each
##' standard.
##' @param std_data a data frame with only the data of the last three injections
##' of the standard samples for a specific measurement file.
##' @param flag.iso_type character string of the isotopic species for which the
##' calibration is to be applied; either "delta.O18" or "delta.H2".
##' @inheritParams wrapper.StandardCalibration
##' @return a list structure with the calibrated standard data along with
##' statistical information on the quality of the calibration approach.
##' @author Thomas Münch
StandardCalibration <- function(std_data, meta.info_std,
    ...
    ix.calibration.standards=
        which(meta.info_std$std.data_corr['calibration',]==1)
    ...
    block.names=
        paste(rep('block',meta.info_std$block_no),
              1:meta.info_std$block_no,sep='')
    ...
    for (ix.block in (1:meta.info_std$block_no)){
        ...
        std_means_blocks=rbind(std_means_blocks,
                               sapply(c(1:meta.info_std$std_no),
                                      ExtractBlockVariable,
                                      std_data,ix.block,flag.iso_type))
    }
    ...
    for (ix.block in (1:meta.info_std$block_no))
    {
        ...
        calib.coeff_blocks[ix.block,'intercept']=-lm.calib$coeff[1]/
            lm.calib$coeff[2]
    ...
    if (flag.Calibration==2){
        ...
        calib.coeff_blocks[final.block,'intercept']=-lm.calib$coeff[1]/
            lm.calib$coeff[2]
        calib.coeff_blocks[final.block,'slope']=1/lm.calib$coeff[2]


        #-----------------------------------------------------------------------
        # get mean line numbers according to the two linear calibrations to
        # obtain a linearly interpolated calibration slope and intercept

        mean.line.no_blocks=vector()

        for (ix.block in (1:meta.info_std$block_no))    
            mean.line.no_blocks=c(mean.line.no_blocks,
                                  mean(sapply(ix.calibration.standards,
                                              ExtractBlockVariable,
                                              std_data,ix.block,'line.numbers'),
                                       na.rm=TRUE))
        ...
        #-----------------------------------------------------------------------
        # linear interpolation of calibration slopes+intercepts to position of
        # inter blocks

        intercept.interpolation=lm(
            c(calib.coeff_blocks['block1','intercept'],
              calib.coeff_blocks[final.block,'intercept']) ~
                c(mean.line.no_blocks['block1'],
                  mean.line.no_blocks[final.block]))
...
##' Calibration of sample data
##'
##' Function to perform a linear calibration of the measured sample isotope
##' data including a drift correction.
##' @param sample_data a data frame with only the data of the samples for a
##' specific measurement file.
##' @param calibration.results the output from
##' \code{\link{wrapper.StandardCalibration}}.
##' @inheritParams CalibratePicarroData
##' @return as \code{sample_data} but with the calibrated data.
##' @author Thomas Münch
SampleCalibration <- function(sample_data, calibration.results,
                              flag.Calibration) {


    #---------------------------------------------------------------------------
    # input from calling routine - drift-correction & calibration coefficients;
    # structure of input depends on calibration scheme
    # -> variable calibration.flag
    ...
    ## --- variable type of calibration coefficients depends on calibration
    ## --- scheme: ---

    ## flag.Calibration=1: calibration intercept/slope is single value
    ## flag.Calibration=2: calibration intercept/slope are vectors where 1st
    ##                     entry is the intercept, 2nd entry the slope to
    ##                     linearly interpolate the calibration coefficients
    ##                     between beginning and end of file

    ## intercept for delta-O18
    calib.intercept_delta.O18=
        calibration.results$calib.coeff_delta.O18$intercept
    ...
    if (flag.Calibration==0)
    {
        ...
        sample_data[,'delta.O18']=
            calib.slope_delta.O18*sample_data[,'delta.O18']+
            calib.intercept_delta.O18

        ## --- delta-H2 data --- ##
        
        sample_data[,'delta.H2']=
            calib.slope_delta.H2*sample_data[,'delta.H2']+
            calib.intercept_delta.H2
    }        
    ...
    else
    {
        ...
        ## linear interpolation of calibration slopes and intercepts to line
        ## numbers
