...
## absolute bias of inter.block standards after correction/calibration procedure;
## given as list
...
sd.residuals_delta.O18_files=calibration.results$sd.residuals_delta.O18_files
sd.residuals_delta.H2_files=calibration.results$sd.residuals_delta.H2_files
...
## name array columns
colnames(bias.arr) <- c(paste(colnames(std_bias[[1]]$delta.O18),'delta.O18',sep='_'),
                        paste(colnames(std_bias[[1]]$delta.O18),'delta.H2',sep='_'))

## convert array to data.frame
df=as.data.frame(bias.arr)

## create output data.frame with meta info of file no. and block no., and bias data
...
CSV.Output <- function(file_data,out_dir,flag.CSV_Output){
    ...
    out_file=paste(paste(file_data$meta.info_file$deviceID,
        file_data$meta.info_file$measurement_date,sep='_'),'csv',sep='.')
